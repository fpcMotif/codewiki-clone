...
export function initHeaderObserver(): void {
  ...
  if (!headerSearch || !heroSection) return;
...
export function initStickyScrollAnimation(): void {
  ...
  const textSectionsContainer = document.querySelector<HTMLElement>('.split-sections');
  const animationContainer = document.querySelector<HTMLElement>('.scroll-animation-container');
  const layers = document.querySelectorAll<HTMLElement>('.sticky-visual .layer');

  if (!stickySection || !textSectionsContainer || !animationContainer || layers.length === 0) return;
  ...
  const handleScroll = () => {
    ...
    const scrollableHeight = textSectionsContainer.offsetHeight - window.innerHeight;
    
    // Calculate scroll progress (0 to 1) within the text container
    const progress = -textRect.top / scrollableHeight;
    const clampedProgress = Math.max(0, Math.min(1, progress));

    if (isNaN(clampedProgress)) return;

    // The total distance the inner container needs to travel
    const totalScrollDistance = (numLayers - 1) * animationContainer.offsetHeight;
