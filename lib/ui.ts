...
export function initThemeSwitcher(): void {
  ...
  const allThemeImages = document.querySelectorAll<HTMLImageElement>('[data-theme-img]');
  ...
  const updateThemeAssets = () => {
    ...
    allThemeImages.forEach(img => {
      const imgTheme = img.dataset.themeImg;
      img.style.display = (imgTheme === (isDark ? 'dark' : 'light')) ? '' : 'none';
    });
  };
